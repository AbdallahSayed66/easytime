...
addTaskForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (window.CapacitorLocalNotifications) {
            const notifDate = new Date(taskData.date);
            if (notifDate > new Date()) {
                window.CapacitorLocalNotifications.schedule({
                    notifications: [
                        {
                            title: 'تذكير بمهمة: ' + taskData.title,
                            body: 'مادة: ' + taskData.subject,
                            id: getNotificationId(savedTaskId),
        ...
        if (window.CapacitorLocalNotifications) {
            ...
            if (examDateTime > new Date()) {
                window.CapacitorLocalNotifications.schedule({
                    notifications: [
                        {
                            title: 'تذكير بامتحان: ' + examData.title,
                            body: 'مادة: ' + examData.subject,
                            id: getNotificationId(savedExamId),
                            schedule: { at: examDateTime }
                        }
                    ]
                });
            }
...
function getNotificationId(firestoreId) {
    let hash = 0;
    for (let i = 0; i < firestoreId.length; i++) {
        hash = (hash << 5) - hash + firestoreId.charCodeAt(i);
        hash |= 0;
    }
...
async function initLocalNotifications() {
    if (window.CapacitorLocalNotifications) {
        try {
            ...
            window.CapacitorLocalNotifications.addListener('localNotificationActionPerformed', (action) => {
                showNotification('الإشعار: ' + action.notification.title);
            });
        } catch (e) {
...
async function loadTimetable() {
    ...
    try {
        ...
        const times = [
            ...
            '2:30-4:00', '4:00-5:30', '5:30-7:00', '7:00-8:30', '8:30-10:00pm'
        ];
        ...
        times.forEach(time => {
            let displayTime = time === '8:30-10:00pm' ? '8:30-10:00' : time;
            tableHTML += `<tr><th>${displayTime}</th>`;
            days.forEach(day => {
                ...
                if (subject) {
                    tableHTML += `
                        ...
                        <div class="subject-time">${displayTime}</div>
...
async function deleteTask(taskId) {
    if (confirm('هل أنت متأكد من حذف هذه المهمة؟')) {
        try {
            ...
            if (window.CapacitorLocalNotifications) {
                window.CapacitorLocalNotifications.cancel({ notifications: [{ id: getNotificationId(taskId) }] });
            }
...
async function deleteExam(examId) {
    if (confirm('هل أنت متأكد من حذف هذا الامتحان؟')) {
        try {
            ...
            if (window.CapacitorLocalNotifications) {
                window.CapacitorLocalNotifications.cancel({ notifications: [{ id: getNotificationId(examId) }] });
            }
...
async function updateTodaySchedule() {
    ...
    try {
        ...
        for (let subject of subjects) {
            const [start] = subject.time.replace('pm', '').split('-');
            let startHour = parseInt(start);
            if (subject.time === '8:30-10:00pm') startHour += 12;
        ...
        if (nextClass) {
            document.getElementById('nextClassTime').textContent = nextClass.time.replace('pm', '').split('-')[0];
        ...
        todaySchedule.innerHTML = subjects.map(subject => {
            const [start, end] = subject.time.replace('pm', '').split('-');
            let subjectStart = parseInt(start);
            let subjectEnd = parseInt(end);
            
            if (subject.time === '8:30-10:00pm') {
                subjectStart += 12;
                subjectEnd += 12;
            } else {
                if (subjectStart < 8) subjectStart += 12;
                if (subjectEnd < 8 || subjectEnd === 12) subjectEnd += 12;
            }
            ...
            return `
                <div class="schedule-item ${statusClass}" data-subject-id="${subject.id}">
                    <div class="subject-time">${start} - ${end}</div>
                    <div class="subject-name">${subject.name}</div>
